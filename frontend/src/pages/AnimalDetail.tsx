...
const AnimalDetail: React.FC = () => {
  ...
  const fetchAnimal = async () => {
    ...
    try {
      setLoading(true);
      const response = await axios.get<Animal>(`http://localhost:8000/api/animals/${id}`, {
    ...
    <div className="container mx-auto p-4">
      ...
      <Card className="overflow-hidden">
        ...
        <CardContent className="p-4 space-y-4">
          ...
          <CardDescription className="text-lg">{animal.description}</CardDescription>
          <div className="flex items-center space-x-4">
            <div className="bg-gray-200 border-2 border-dashed rounded-xl w-16 h-16" >
              <img
                src={"https://github.com/prosabd/zoo-symfony-react/releases/download/0.0.0/" + (animal.family).toLowerCase().replace(' ', '_') + ".svg" ? "https://github.com/prosabd/zoo-symfony-react/releases/download/0.0.0/" + (animal.family).toLowerCase().replace(' ', '_') + ".svg" : "@/public/user_assets/families_images/" + (animal.family).toLowerCase().replace(' ', '_') + ".{" + ['jpg', 'png', 'jpeg', 'svg'].join('|') + "}" }
                alt=""
              />
            </div>
            <div>
              <p className="text-sm text-muted-foreground">Family: {animal.family}</p>
            </div>
          </div>
        </CardContent>
      </Card>
